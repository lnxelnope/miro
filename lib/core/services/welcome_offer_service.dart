...
///
/// **Trigger:** ครั้งแรกที่ Energy หมด (จัดการโดย EnergyService.checkAndHandleFirstEnergyEmpty)
/// **Limit:** ซื้อได้เพียง 1 package ต่อ 1 device (ไม่ว่าจะเป็น package ไหน)
class WelcomeOfferService {
  static const String _keyAiUsageCount =
      'ai_usage_count'; // [DEPRECATED] ไม่ใช้แล้ว - ใช้ first_energy_empty แทน
  ...
  static const String _keyPurchasedPackage =
      'welcome_package_purchased'; // เก็บว่าซื้อ package ไหนไปแล้ว
  static const int triggerCount = 10; // [DEPRECATED] ไม่ใช้แล้ว
  ...
  static Future<Duration?> getRemainingTime() async {
    ...
    final expiresAt =
        DateTime.fromMillisecondsSinceEpoch(firstUsageMs).add(offerDuration);
  ...
  ///
  /// **[DEPRECATED]** ไม่ใช้แล้ว - Welcome Offer จะถูก trigger โดย EnergyService.checkAndHandleFirstEnergyEmpty
  /// 
  /// Returns: true ถ้าเพิ่งเริ่ม timer (ครบ 10 ครั้งพอดี)
  @Deprecated('Welcome Offer is now triggered by first energy empty, not AI usage count')
  static Future<bool> incrementUsageAndCheckTimer() async {
    ...
    if (newCount >= triggerCount) {
      await prefs.setInt(
          _keyFirstAiUsage, DateTime.now().millisecondsSinceEpoch);
  ...
  @Deprecated('No longer used - Welcome Offer is triggered by first energy empty')
  ...
  /// 
  /// **[DEPRECATED]** ไม่ใช้แล้ว - Welcome Offer ถูก trigger เมื่อ Energy หมดครั้งแรก
  @Deprecated('No longer used - Welcome Offer is triggered by first energy empty')
