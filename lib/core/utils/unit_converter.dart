...
class UnitConverter {
  ...
  static const _weight = [
    ServingUnit(key: 'g',   label: 'gram (g)',       shortLabel: 'g',   category: UnitCategory.weight, toBaseFactor: 1),
    ServingUnit(key: 'kg',  label: 'kilogram (kg)',  shortLabel: 'kg',  category: UnitCategory.weight, toBaseFactor: 1000),
    ServingUnit(key: 'mg',  label: 'milligram (mg)', shortLabel: 'mg',  category: UnitCategory.weight, toBaseFactor: 0.001),
    ServingUnit(key: 'oz',  label: 'ounce (oz)',     shortLabel: 'oz',  category: UnitCategory.weight, toBaseFactor: 28.3495),
    ServingUnit(key: 'lbs', label: 'pound (lbs)',    shortLabel: 'lbs', category: UnitCategory.weight, toBaseFactor: 453.592),
  ];

  // ---------- Volume (base = ml) ----------
  static const _volume = [
    ServingUnit(key: 'ml',    label: 'milliliter (ml)',   shortLabel: 'ml',    category: UnitCategory.volume, toBaseFactor: 1),
    ServingUnit(key: 'l',     label: 'liter (l)',         shortLabel: 'l',     category: UnitCategory.volume, toBaseFactor: 1000),
    ServingUnit(key: 'fl oz', label: 'fluid ounce (fl oz)', shortLabel: 'fl oz', category: UnitCategory.volume, toBaseFactor: 29.5735),
    ServingUnit(key: 'cup',   label: 'cup',               shortLabel: 'cup',   category: UnitCategory.volume, toBaseFactor: 236.588),
    ServingUnit(key: 'tbsp',  label: 'tablespoon (tbsp)', shortLabel: 'tbsp',  category: UnitCategory.volume, toBaseFactor: 15),
    ServingUnit(key: 'tsp',   label: 'teaspoon (tsp)',    shortLabel: 'tsp',   category: UnitCategory.volume, toBaseFactor: 5),
  ];

  // ---------- Count / Portion ----------
  static const _count = [
    ServingUnit(key: 'serving', label: 'serving',  shortLabel: 'srv',   category: UnitCategory.count),
    ServingUnit(key: 'piece',   label: 'piece',    shortLabel: 'pc',    category: UnitCategory.count),
    ServingUnit(key: 'slice',   label: 'slice',    shortLabel: 'slice', category: UnitCategory.count),
    ServingUnit(key: 'plate',   label: 'plate',    shortLabel: 'plate', category: UnitCategory.count),
    ServingUnit(key: 'bowl',    label: 'bowl',     shortLabel: 'bowl',  category: UnitCategory.count),
    ServingUnit(key: 'cup_c',   label: 'cup (count)', shortLabel: 'cup', category: UnitCategory.count),
    ServingUnit(key: 'glass',   label: 'glass',    shortLabel: 'glass', category: UnitCategory.count),
    ServingUnit(key: 'egg',     label: 'egg',      shortLabel: 'egg',   category: UnitCategory.count),
    ServingUnit(key: 'ball',    label: 'ball',     shortLabel: 'ball',  category: UnitCategory.count),
    ServingUnit(key: 'fruit',   label: 'fruit',    shortLabel: 'fruit', category: UnitCategory.count),
    ServingUnit(key: 'skewer',  label: 'skewer',   shortLabel: 'skw',   category: UnitCategory.count),
    ServingUnit(key: 'whole',   label: 'whole',    shortLabel: 'whole', category: UnitCategory.count),
    ServingUnit(key: 'sheet',   label: 'sheet',    shortLabel: 'sht',   category: UnitCategory.count),
    ServingUnit(key: 'pair',    label: 'pair',     shortLabel: 'pair',  category: UnitCategory.count),
    ServingUnit(key: 'bunch',   label: 'bunch',    shortLabel: 'bnch',  category: UnitCategory.count),
    ServingUnit(key: 'leaf',    label: 'leaf',     shortLabel: 'leaf',  category: UnitCategory.count),
    ServingUnit(key: 'stick',   label: 'stick',    shortLabel: 'stk',   category: UnitCategory.count),
    ServingUnit(key: 'scoop',   label: 'scoop',    shortLabel: 'scoop', category: UnitCategory.count),
    ServingUnit(key: 'handful', label: 'handful',  shortLabel: 'hndfl', category: UnitCategory.count),
  ];

  // ---------- Package ----------
  static const _package = [
    ServingUnit(key: 'pack',    label: 'pack',     shortLabel: 'pack',  category: UnitCategory.package),
    ServingUnit(key: 'bag',     label: 'bag',      shortLabel: 'bag',   category: UnitCategory.package),
    ServingUnit(key: 'wrap',    label: 'wrap',     shortLabel: 'wrap',  category: UnitCategory.package),
    ServingUnit(key: 'box',     label: 'box',      shortLabel: 'box',   category: UnitCategory.package),
    ServingUnit(key: 'can',     label: 'can',      shortLabel: 'can',   category: UnitCategory.package),
    ServingUnit(key: 'bottle',  label: 'bottle',   shortLabel: 'btl',   category: UnitCategory.package),
    ServingUnit(key: 'bar',     label: 'bar',      shortLabel: 'bar',   category: UnitCategory.package),
  ];
  ...
  static double? convert(double quantity, {required String from, required String to}) {
  ...
  static List<DropdownMenuItem<String>> get allDropdownItems => [
    ...
    const DropdownMenuItem(value: '', enabled: false,
      child: Text('─── Weight ───', style: TextStyle(fontSize: 12, color: Colors.grey))),
    ..._weight.map((u) => DropdownMenuItem(value: u.key, child: Text(u.label))),

    const DropdownMenuItem(value: '', enabled: false,
      child: Text('─── Volume ───', style: TextStyle(fontSize: 12, color: Colors.grey))),
    ..._volume.map((u) => DropdownMenuItem(value: u.key, child: Text(u.label))),

    const DropdownMenuItem(value: '', enabled: false,
      child: Text('─── Count / Portion ───', style: TextStyle(fontSize: 12, color: Colors.grey))),
    ..._count.map((u) => DropdownMenuItem(value: u.key, child: Text(u.label))),

    const DropdownMenuItem(value: '', enabled: false,
      child: Text('─── Package ───', style: TextStyle(fontSize: 12, color: Colors.grey))),
    ..._package.map((u) => DropdownMenuItem(value: u.key, child: Text(u.label))),
  ];
  ...
  static List<DropdownMenuItem<String>> get compactDropdownItems => [
    ..._weight.map((u) => DropdownMenuItem(value: u.key, child: Text(u.shortLabel))),
    ..._volume.map((u) => DropdownMenuItem(value: u.key, child: Text(u.shortLabel))),
    ..._count.map((u) => DropdownMenuItem(value: u.key, child: Text(u.shortLabel))),
    ..._package.map((u) => DropdownMenuItem(value: u.key, child: Text(u.shortLabel))),
  ];
  ...
  static String displayUnit(String unitKey, String locale) {
    ...
    if (names == null) return displayLabel(unitKey); // fallback to original label
