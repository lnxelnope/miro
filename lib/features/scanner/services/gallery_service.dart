...
class GalleryService {
  ...
  Future<List<AssetEntity>> fetchNewImages({DateTime? after}) async {
    ...
    AppLogger.info('Fetching images (limit $fetchCount from $totalAssets images)...');
    
    // ดึงรูปภาพตาม limit (เรียงใหม่ไปเก่าแล้ว)
    List<AssetEntity> media = await albums[0].getAssetListRange(start: 0, end: fetchCount);
    ...
    if (after != null) {
      ...
      media = media.where((asset) => asset.createDateTime.isAfter(after)).toList();
      AppLogger.info('Filtered by date (after ${after.toString()}): $beforeCount → ${media.length} images');
    }
